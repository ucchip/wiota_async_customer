...
{
    rt_list_t *head = &(g_wiota_manager.freq_list.node);
    ...
    {
        g_wiota_manager.current_freq_node = rt_list_entry(tmp, t_freq_list_manager, node);
    }
    else
    {
        tmp = g_wiota_manager.current_freq_node->node.next;
        if (tmp == head)
        {
