...
void wiota_data_show(void)
{
    ...
    for (; i < max; i++)
    {
        idx = i % WIOTA_DATA_NUM;
        if ((RT_NULL != wiota_save_data[idx].data) && (wiota_save_data[idx].data_len > 0))
        {
            // at_server_printf("+WIOTARECV,%d,%d,", wiota_save_data[i % WIOTA_DATA_NUM].type, wiota_save_data[i % WIOTA_DATA_NUM].data_len);
            at_server_printf("+WIOTARECV,-%d,%d,%d,%d,%d,",
                wiota_save_data[idx].rssi,
                wiota_save_data[idx].snr,
                wiota_save_data[idx].type,
                wiota_save_data[idx].result,
                wiota_save_data[idx].data_len);
            at_send_data(wiota_save_data[idx].data, wiota_save_data[idx].data_len);
            at_server_printfln("");
            rt_free(wiota_save_data[idx].data);
        }
        wiota_save_data[idx].data = RT_NULL;
        wiota_save_data[idx].data_len = 0;
    }
    wiota_save_index = -1;
}
