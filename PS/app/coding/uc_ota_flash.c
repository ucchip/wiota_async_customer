#include <rtthread.h>
#if 1//defined(WIOTA_APP_DEMO) || defined(AT_WIOTA_GATEWAY_API)
#include <rtdevice.h>
#include <rthw.h>
...
static void uc_wiota_suspend_connect(void)
{
    uc_wiota_set_recv_mode(UC_AUTO_RECV_STOP);
    rt_thread_mdelay(20);
}

static void uc_wiota_recover_connect(void)
{
...
extern void set_download_file_size(int filesize);
static void uc_wiota_set_download_file_size(int filesize)
{
    set_download_file_size(filesize);
}
